...
from models import *
from models.resnet import PonderGRU, PonderGRUAction, RegularizationLoss
...
parser.add_argument('--model', type=str, default='PonderGRU', choices=['PonderGRU', 'PonderGRUAction'])
...
def main():
  ...
  if args.model == 'PonderGRU':
      model = PonderGRU(n_elems=512, n_hidden=100, max_steps=10).to('cuda')
      checkpoint = torch.load('./checkpoint/ckpt_colorjitter_PonderGRU_10step_0.3.pth')
      model.load_state_dict(checkpoint['net'])
  elif args.model == 'PonderGRUAction':
      model = PonderGRUAction(n_elems=512, n_hidden=512, max_steps=10).to('cuda')
